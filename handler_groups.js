...
function groupHomeKeyboard(chatId) {
  return Markup.inlineKeyboard([
    [Markup.button.callback('⚙️ إعدادات المجموعة', `settings_${chatId}`),
     Markup.button.callback('👥 المشرفون',          `admins_${chatId}`)],
...
module.exports = function setupGroupHandlers(bot) {
  ...
  bot.on('my_chat_member', async (ctx) => {
    ...
    if (joined) {
      if (isChannel) {
        ...
        try { await ctx.replyWithMarkdown(`📢 *شكراً لإضافتي لقناة ${chat.title}!*\n\n👑 المالك: \`${channel.ownerUsername || 'غير محدد'}\`\n🛡️ سأتابع مشتركي القناة.`); } catch { }
      } else {
        ...
        try {
          ...
          if (me.status === 'administrator' || newStat === 'administrator') promoted = await promoteUser(bot, chat.id, from.id);
        } catch { }
        ...
        await ctx.replyWithMarkdown(
          ...
          `🛡️ جاهز للإدارة!\n📌 *الميزات:* كلمات محظورة | تحذيرات | طلبات الانضمام | حماية المجتمع\n\n_استخدم /settings للإعدادات_`,
  ...
  bot.on('chat_member', async (ctx) => {
    ...
    if (newM.status === 'member' && (oldM.status === 'left' || oldM.status === 'kicked')) {
      ...
      const msg = g.welcomeMessage
        .replace('{name}', u.first_name || '').replace('{group}', chat.title || 'المجموعة')
  ...
  bot.on('message', async (ctx, next) => {
    ...
    if (count < found.threshold) {
      await ctx.reply(
        `⚠️ ${userName} — رسالتك حُذفت لاحتوائها على كلمة محظورة.\n` +
        `⚡ الانتهاك ${count}/${found.threshold} — الإجراء عند الوصول: *${arAction(found.action)}*`,
        { parse_mode: 'Markdown' }
      );
      return next();
    }
    ...
    if (found.action === 'warn') {
      ...
      if (warns.length >= g.maxWarns) {
        ...
        await ctx.reply(`🚫 ${userName} — حظر تلقائي بعد ${g.maxWarns} تحذيرات (كلمة: "${found.word}")`);
      } else {
  ...
  bot.action(/^group_home_(-?\d+)$/, async (ctx) => {
    ...
    await ctx.editMessageText(`🤖 *إدارة ${g.title}*\n\nاختر من القائمة:`,
      { parse_mode: 'Markdown', ...groupHomeKeyboard(g.chatId) });
  });
...
function arAction(a) { return { warn: 'تحذير', mute: 'كتم', kick: 'طرد', ban: 'حظر' }[a] || a; }
