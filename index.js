...
//  + تخزين في الذاكرة مع backup/restore عبر JSON
// ============================================================
  ...
  try {
    ...
    for (const r of expired) {
      try {
        ...
        console.log(`✅ رُفع ${r.type} عن ${r.user_id} في ${r.chat_id}`);
      } catch (e) {
        console.error(`❌ فشل رفع ${r.type}:`, e.message);
      }
    }
  } catch (e) { console.error('❌ scheduler error:', e.message); }
}, 60 * 1000);

// ── Scheduler: حفظ تلقائي للبيانات كل 30 دقيقة ──────────────
setInterval(() => {
  console.log('💾 حفظ تلقائي للبيانات...');
  db.saveData();
}, 30 * 60 * 1000);
...
// ── مزامنة المجموعات عند بدء التشغيل ────────────────────────
  ...
  for (const g of groups) {
    ...
    try {
      ...
      if (member.status === 'left' || member.status === 'kicked') {
        await db.deleteGroup(chatId); removed++;
      } else {
        try {
          const admins = await bot.telegram.getChatAdministrators(chatId);
          const owner  = admins.find(a => a.status === 'creator');
          if (owner) { g.ownerId = owner.user.id; g.ownerUsername = owner.user.username || owner.user.first_name; db.scheduleSync(g); }
          updated++;
        } catch {}
      }
    } catch {}
...
if (RENDER_URL) {
  ...
  app.listen(PORT, async () => {
    ...
    try {
      await bot.telegram.setWebhook(WEBHOOK_URL, { allowed_updates: ALLOWED_UPDATES, drop_pending_updates: true });
  ...
  setInterval(async () => {
    try { const res = await fetch(`${RENDER_URL}/health`); const data = await res.json(); console.log(`🏓 Keep-alive — uptime: ${Math.floor(data.uptime)}s`); } catch (e) { console.warn('⚠️ Keep-alive فشل:', e.message); }
  }, 14 * 60 * 1000);
  ...
  async function startPolling(retries = 0) {
    try {
      ...
      await bot.launch({ allowedUpdates: ALLOWED_UPDATES, dropPendingUpdates: true });
      const botInfo = await bot.telegram.getMe();
      console.log(`✅ البوت يعمل بوضع Polling: @${botInfo.username}`);
      setTimeout(() => syncBotChats(botInfo).catch(console.error), 5000);
    } catch (err) {
      ...
      } else { console.error('❌ فشل تشغيل البوت:', err.message); process.exit(1); }
    }
  }

  startPolling();
  process.once('SIGINT',  () => { db.saveData(); bot.stop('SIGINT');  process.exit(0); });
  process.once('SIGTERM', () => { db.saveData(); bot.stop('SIGTERM'); process.exit(0); });
}
