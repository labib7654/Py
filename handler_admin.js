...
function memberActionsKeyboard(targetId, chatId, backCb) {
  return Markup.inlineKeyboard([
    [Markup.button.callback('🔇 كتم',      `mute_${targetId}_${chatId}`), Markup.button.callback('🔊 رفع كتم', `unmute_${targetId}_${chatId}`)],
    [Markup.button.callback('⚠️ تحذير',    `warn_${targetId}_${chatId}`), Markup.button.callback('👢 طرد',      `kick_${targetId}_${chatId}`)],
    [Markup.button.callback('🚫 حظر',      `ban_${targetId}_${chatId}`),  Markup.button.callback('✅ رفع حظر', `unban_${targetId}_${chatId}`)],
    [Markup.button.callback('📋 معلومات',  `info_${targetId}_${chatId}`), Markup.button.callback('🔙 رجوع',    backCb || 'cancel')],
  ]);
}
...
module.exports = function setupAdminHandlers(bot) {
  ...
  bot.command('ban', async (ctx) => {
    ...
    try {
      ...
      await ctx.replyWithMarkdown(`🚫 *تم الحظر*\n👤 ${target.username ? `@${target.username}` : target.firstName}\n👮 @${ctx.from.username || ctx.from.first_name}\n📝 ${reason}`, Markup.inlineKeyboard([[Markup.button.callback('✅ رفع الحظر', `unban_${target.id}_${chatId}`)]]));
  ...
  bot.command('mute', async (ctx) => {
    ...
    try {
      ...
      await ctx.replyWithMarkdown(`🔇 *تم الكتم*\n👤 ${target.username ? `@${target.username}` : target.firstName}`, Markup.inlineKeyboard([[Markup.button.callback('🔊 رفع الكتم', `unmute_${target.id}_${chatId}`)]]));
    } catch (e) { await ctx.reply(`❌ ${e.message}`); }
  });
  ...
  bot.command('warn', async (ctx) => {
    ...
    if (warns.length >= g.maxWarns) {
      ...
      await ctx.replyWithMarkdown(`⚠️ *تحذير ${warns.length}/${g.maxWarns}*\n👤 ${target.username ? `@${target.username}` : target.firstName}\n📝 ${reason}`, Markup.inlineKeyboard([[Markup.button.callback('🗑️ حذف التحذيرات', `clearwarns_${target.id}_${chatId}`)]]));
    }
  });
  ...
  bot.command('manage', async (ctx) => {
    ...
    await ctx.replyWithMarkdown(`👤 *إدارة المستخدم*\n\nالاسم: ${target.firstName || '—'}\n@${target.username || '—'}\nالآيدي: \`${target.id}\`\nالتحذيرات: \`${warns}/${g?.maxWarns || 3}\``, memberActionsKeyboard(target.id, chatId, 'cancel'));
  });
  ...
  bot.action(/^unmute_(\d+)_(-?\d+)$/, async (ctx) => {
    const [tid, cid] = [Number(ctx.match[1]), Number(ctx.match[2])];
    if (!isDeveloper(ctx) && !await isAdmin(bot, cid, ctx.from.id)) return ctx.answerCbQuery('❌ للمشرفين فقط!', { show_alert: true });
    try { await bot.telegram.restrictChatMember(cid, tid, { permissions: { can_send_messages: true, can_send_audios: true, can_send_documents: true, can_send_photos: true, can_send_videos: true, can_send_video_notes: true, can_send_voice_notes: true, can_send_polls: true, can_send_other_messages: true, can_add_web_page_previews: true } }); const g = db.getGroup(cid); if (g) g.mutedUsers.delete(tid); await ctx.answerCbQuery('✅ تم رفع الكتم!', { show_alert: true }); }
    catch (e) { await ctx.answerCbQuery(`❌ ${e.message}`, { show_alert: true }); }
  });
  ...
  bot.action(/^ban_(\d+)_(-?\d+)$/, async (ctx) => {
    const [tid, cid] = [Number(ctx.match[1]), Number(ctx.match[2])];
    if (!isDeveloper(ctx) && !await isAdmin(bot, cid, ctx.from.id)) return ctx.answerCbQuery('❌ للمشرفين فقط!', { show_alert: true });
    try { await bot.telegram.banChatMember(cid, tid); const g = db.getGroup(cid); if (g) g.bannedUsers.add(tid); await ctx.answerCbQuery('✅ تم الحظر!', { show_alert: true }); }
    catch (e) { await ctx.answerCbQuery(`❌ ${e.message}`, { show_alert: true }); }
  });
  ...
  bot.action(/^kick_(\d+)_(-?\d+)$/, async (ctx) => {
    const [tid, cid] = [Number(ctx.match[1]), Number(ctx.match[2])];
    if (!isDeveloper(ctx) && !await isAdmin(bot, cid, ctx.from.id)) return ctx.answerCbQuery('❌ للمشرفين فقط!', { show_alert: true });
    try { await bot.telegram.banChatMember(cid, tid); setTimeout(() => bot.telegram.unbanChatMember(cid, tid).catch(() => { }), 1500); await ctx.answerCbQuery('✅ تم الطرد!', { show_alert: true }); }
    catch (e) { await ctx.answerCbQuery(`❌ ${e.message}`, { show_alert: true }); }
  });
  ...
  bot.action(/^warn_(\d+)_(-?\d+)$/, async (ctx) => {
    ...
    const warns = g.warns.get(tid); warns.push({ reason: 'تحذير يدوي', warnedBy: ctx.from.id, warnedAt: new Date() });
    if (warns.length >= g.maxWarns) { try { await bot.telegram.banChatMember(cid, tid); g.bannedUsers.add(tid); g.warns.delete(tid); } catch { } await ctx.answerCbQuery(`🚫 حظر بعد ${g.maxWarns} تحذيرات!`, { show_alert: true }); }
