...
module.exports = function setupDeveloper(bot) {
  ...
  bot.action('dev_groups', async (ctx) => {
    ...
    const btns = groups.slice(0, 10).map(g =>
      [Markup.button.callback(`👥 ${g.title.slice(0, 26)}`, `dev_grp_${g.chatId}`)]
    );
    btns.push([Markup.button.callback('🔙 رجوع', 'dev_back')]);
    await ctx.editMessageText(`👥 *قائمة المجموعات* (${groups.length})`,
  ...
  bot.action(/^dev_grp_(-?\d+)$/, async (ctx) => {
    ...
    await ctx.editMessageText(
      ...
      `➕ أضافه: \`${g.addedByUsername}\`\n📅 \`${g.addedAt.toLocaleDateString('ar-SA')}\`\n` +
      ...
      `🛡️ مكافحة السبام: ${g.antiSpam ? '✅' : '❌'} | 👋 الترحيب: ${g.welcomeEnabled ? '✅' : '❌'}`,
      {
        ...
        ...Markup.inlineKeyboard([
          [Markup.button.callback('👥 الأعضاء',          `dev_members_${chatId}`),
           Markup.button.callback('⚠️ المحذَّرون',       `dev_warned_${chatId}`)],
          [Markup.button.callback('🔇 المكتومون',        `dev_muted_${chatId}`),
           Markup.button.callback('🚫 المحظورون',        `dev_banned_grp_${chatId}`)],
  ...
  bot.action(/^dev_members_(-?\d+)$/, async (ctx) => {
    ...
    if (members.length > 0) {
      ...
      members.slice(0, 8).forEach(m => {
        ...
        const warns = g.warns.get(m.userId)?.length || 0;
        const icons = `${warns > 0 ? `⚠️${warns}` : ''}${g.mutedUsers.has(m.userId) ? '🔇' : ''}${g.bannedUsers.has(m.userId) ? '🚫' : ''}`;
  ...
  bot.action(/^dev_mact_(\d+)_(-?\d+)$/, async (ctx) => {
    ...
    const role  = { owner: '👑 مالك', admin: '👮 مشرف', member: '👤 عضو' }[m?.role] || '👤';
  ...
  bot.action('dev_channels', async (ctx) => {
    ...
    const btns = chs.slice(0, 10).map(c =>
      [Markup.button.callback(`📢 ${c.title.slice(0, 26)}`, `dev_ch_${c.chatId}`)]
    );
    btns.push([Markup.button.callback('🔙 رجوع', 'dev_back')]);
  ...
  bot.action(/^dev_ch_(-?\d+)$/, async (ctx) => {
    ...
    await ctx.editMessageText(
      ...
      `📅 \`${c.addedAt.toLocaleDateString('ar-SA')}\`\n👥 المشتركون: \`${c.subscribers.size}\``,
  ...
  bot.action('dev_communities', async (ctx) => {
    ...
    const btns = coms.map(c =>
      [Markup.button.callback(`🌐 ${c.title.slice(0, 24)} (${c.subGroups.size})`, `dev_com_${c.communityId}`)]
    );
    btns.push([Markup.button.callback('🔙 رجوع', 'dev_back')]);
  ...
  bot.action(/^dev_com_(-?\d+)$/, async (ctx) => {
    ...
    const subNames = [...c.subGroups].map(id => {
      const g = db.getGroup(id);
      return g ? `• ${g.title} \`[${id}]\`` : `• \`${id}\``;
    }).join('\n') || '_لا توجد مجموعات فرعية_';
    await ctx.editMessageText(
      `🌐 *${c.title}*\n\n🆔 \`${c.communityId}\`\n📁 المجموعات الفرعية:\n${subNames}\n\n` +
      `⚙️ الحد الأقصى للانضمام: \`${c.maxGroupJoins}\`\n🛡️ الحماية: ${c.enabled ? '✅' : '❌'}`,
  ...
  bot.command('community', async (ctx) => {
    ...
    if (args[0] === 'addgroup') {
      ...
      const c = db.getCommunity(comId);
      if (!c) return ctx.reply('❌ المجتمع غير موجود!');
    ...
    if (args[0] === 'setlimit') {
      ...
      const c = db.getCommunity(comId);
      if (!c || !limit) return ctx.reply('❌ مثال: /community setlimit 123456 1');
      c.maxGroupJoins = limit;
      return ctx.replyWithMarkdown(`✅ الحد الأقصى: \`${limit}\` مجموعة لمجتمع *${c.title}*`);
    }
  ...
  bot.command('bcastone', async (ctx) => {
    ...
    const chatId = Number(parts[1]); const text = parts.slice(2).join(' ');
  ...
  bot.command('gban', async (ctx) => {
    if (!isDeveloper(ctx)) return;
    const args = ctx.message.text.split(' ').slice(1);
    const targetId = Number(args[0]); const reason = args.slice(1).join(' ') || 'لا يوجد سبب';
  ...
  bot.command('userinfo', async (ctx) => {
    ...
    const groupNames = [...user.groups].map(id => db.getGroup(id)?.title || String(id)).join(', ') || '—';
    await ctx.replyWithMarkdown(
      `👤 *معلومات المستخدم*\n\n🆔 \`${user.userId}\`\n📛 ${user.firstName}\n🔗 ${user.username ? `@${user.username}` : '—'}\n` +
      `📅 \`${user.firstSeen.toLocaleDateString('ar-SA')}\`\n🌍 محظور: ${user.globalBanned ? `✅ — ${user.bannedReason}` : '❌'}\n👥 ${groupNames}`
    );
  });
  ...
  bot.action(/^dev_joinreqs_(-?\d+)$/, async (ctx) => {
    ...
    btns.push([Markup.button.callback('🔙 رجوع', `dev_grp_${chatId}`)]);
    await ctx.editMessageText(`📨 *طلبات الانضمام* (${pending.length} معلقة)\n📌 *${g.title}*`,
      { parse_mode: 'Markdown', ...Markup.inlineKeyboard(btns) });
  });
  ...
  bot.action('help_menu', async (ctx) => {
    ...
    await ctx.editMessageText(
      ...
      '🌐 *المجتمعات (المطور):*\n`/community create` `/community addgroup` `/community setlimit`',
      { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('🔙 رجوع', 'help_back')]]) }
    );
  });
